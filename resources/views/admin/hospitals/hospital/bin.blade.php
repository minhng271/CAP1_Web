...
@section('content')
    <div class="content">
        <div id="content" class="container-fluid">
            ...
            @endif
            @if (session('delete'))
                <div class="alert alert-success" style="margin-bottom: 1px">Đã xóa <b>{{ session('delete') }}</b> thành
                    công !!!</div>
            @endif
            @if (session('restore'))
                <div class="alert alert-success" style="margin-bottom: 1px">Khôi phục <b>{{ session('restore') }}</b> thành
                    công !!!</div>
            @endif
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ url('admin/hospital/restore/' . $item->id) }}"
                                                ...
                                                onclick="return confirm('Khôi phục tài khoản?')"><i data-toggle="tooltip"
                                                    data-placement="top" title="Khôi Phục" name="edit">
                                                    <i class="far fa-window-restore"></i>
                                            </a>
                                            <a href="{{ url('admin/hospital/delete-bin/' . $item->id) }}"
                                                class="btn btn-danger btn-sm rounded-0 text-white" type="button"
                                                title="Xóa Vĩnh Viễn" onclick="return confirm('Xóa vĩnh viễn?')"><i
                                                    class="fa fa-trash"></i>
                                            </a>
                                        </td>
                                    </tr>
