...
@section('content')
    ...
    <main class="content">
        <div class="container-fluid p-0">
            @if (session('restore'))
                <span style=" margin-top: 4px; " class="alert alert-success">Khôi phục loại bệnh: <b>{{ session('restore') }}</b> Thành Công</span>
            @endif
            @if (session('delete'))
                <span style=" margin-top: 4px; " class="alert alert-success">Xóa vĩnh viễn loại bệnh: <b>{{ session('delete') }}</b> thành công</span>
            @endif
            <div class="row">
                <div class="col-12 col-lg-12 col-xxl-12">
                    <div class="card-header  d-flex justify-content-between">
                        <h3 style="text-transform: uppercase; font-weight: 600;">Danh sách Loại Bệnh đã xóa</h3>
                                    ...
                                    @foreach ($list_disease as $disease)
                                        ...
                                        <tr>
                                            ...
                                            <td class="d-none d-md-table-cell" id="done[]"
                                                ...
                                                <a href="{{ url('vaccine/restore-disease', ['id' =>  $disease['id']]) }}"
                                                    onclick="return confirm('KHÔI PHỤC LOẠI BỆNH NÀY')">
                                                    <i class="fas fa-window-restore"></i>
                                                </a>
                                            </td>
                                            <td class="d-none d-md-table-cell" id="delete[]"
                                                style="border-right: 1px solid rgb(207, 207, 207) ">
                                                <a href="{{ url('vaccine/delete-disease-bin', ['id' => $disease['id']]) }}"
                                                    onclick="return confirm('XÓA VĨNH VIỄN LOẠI BỆNH NÀY')">
