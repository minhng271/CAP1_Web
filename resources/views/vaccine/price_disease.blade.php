...
<main class="content">
    @if (session('name_vac'))
                <div class="alert alert-success" style="margin-bottom:1px;">Cập nhật giá tiền tiêm vaccine cho bệnh <b>{{ session('name_vac') }}</b></div>
            @endif
                            ...
                            @if ($price_dis)
                                @foreach ($price_dis as $item)
                                    ...
                                    <tr>
                                        ...
                                        <td class="d-none d-xl-table-cell">{{ number_format($item->price_vac) }} đ</td>
                                        <td class="d-none d-xl-table-cell">
                                        <a class="btn btn-warning" href="{{ url('vaccine/price_vac_edit',['id'=>$item->id]) }}">Chỉnh Sửa</a>
                                        </td>
                                    </tr>
                                @endforeach
                            @else
